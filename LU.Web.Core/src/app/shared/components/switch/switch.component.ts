...
@Component({
	...
	<div #container class="bootstrap-switch-container"  [@statusChange]="_statusStr" >
        <span #on class="bootstrap-switch-handle-on" [(ngClass)]="onColor" [ngStyle]="{'min-width': _minWidth +'px'}">{{onText}}</span>
        <span #mid class="bootstrap-switch-label"></span>
        <span #off class="bootstrap-switch-handle-off " [(ngClass)]="offColor">{{offText}}</span>
    </div>
</div>`,
    styles: [`
.bootstrap-switch {
...
export class SwitchComponent implements OnChanges, AfterViewInit {
    ...
    private _calculateWidth(): void {
        if (this._onSpan) {
            ...
            this.renderer.setElementStyle(this._midSpan.nativeElement, 'width', (width - 50).toString() + 'px');
            ...
            this.renderer.setElementStyle(this._main.nativeElement, 'width', (width * 1.25).toString() + 'px');
        }
    }
